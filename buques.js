...
function pickBestMatch(records, query) {
  ...
  const exact = records.find(record => normalizeVesselName(record.name) === normalizedQuery);
  if (exact) return exact;

  const startsWith = records.find(record => normalizeVesselName(record.name).startsWith(normalizedQuery));
...
function renderVesselDetail(vessel) {
  const name = vessel?.name || vessel?.official_name || '-';
  const flag = vessel?.flag || '-';
  const nationality = vessel?.flag || '-';
  const imo = vessel?.imo || '-';

  detailVesselName.textContent = name;
  detailVesselFlag.textContent = flag;
  detailVesselNationality.textContent = nationality;
  detailVesselImo.textContent = imo;
