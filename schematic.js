...
const vesselOutput = document.getElementById('vesselOutput');
...
const btnVesselSearch = document.getElementById('btnVesselSearch');
const btnCopyVessel = document.getElementById('btnCopyVessel');
const btnClear = document.getElementById('btnClear');

function clean(value) { return String(value ?? '').replace(/\r/g, '').trim(); }
function normalize(value) { return clean(value).toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/\s+/g, ' '); }
function pad2(value) { return String(value).padStart(2, '0'); }

function abbreviateTerminal(value) {
  const text = clean(value);
  if (!text) return '';
  const normalized = normalize(text).replace(/\./g, '').replace(/,/g, '').replace(/\s+/g, ' ').trim();
  const aliases = {
...
function parseDateTime(value) {
  ...
  if (match) {
    ...
    const months = { jan:0, ene:0, feb:1, mar:2, apr:3, abr:3, may:4, jun:5, jul:6, aug:7, ago:7, sep:8, oct:9, nov:10, dec:11, dic:11 };
...
function formatDateTimeMX(date) { if (!date || Number.isNaN(date.getTime())) return ''; return `${pad2(date.getDate())}/${pad2(date.getMonth() + 1)}/${date.getFullYear()} ${pad2(date.getHours())}:${pad2(date.getMinutes())}`; }
function formatShortScheduleDate(date) { if (!date || Number.isNaN(date.getTime())) return ''; const monthNames = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']; return `${pad2(date.getDate())} ${monthNames[date.getMonth()]} ${pad2(date.getHours())}:${pad2(date.getMinutes())}`; }
function formatDateMX(date) { if (!date || Number.isNaN(date.getTime())) return ''; return `${pad2(date.getDate())}/${pad2(date.getMonth()+1)}/${date.getFullYear()}`; }
...
function saveSchematic(records) { localStorage.setItem(STORAGE_KEY, JSON.stringify({ loadedAt: new Date().toISOString(), records })); }
function loadSchematic() { try { const raw = localStorage.getItem(STORAGE_KEY); return raw ? JSON.parse(raw) : null; } catch { return null; } }
function updateStatus(payload) { if (!payload || !payload.records?.length) { processStatus.textContent = 'Sin schematic cargado.'; return; } const loadedAt = new Date(payload.loadedAt).toLocaleString('es-MX'); processStatus.textContent = `Schematic vigente: ${payload.records.length} registro(s) | Actualizado: ${loadedAt}`; }

function findHeaderRow(rows) { for (let i = 0; i < rows.length; i += 1) { const normalized = rows[i].map(normalize); if (normalized.includes('vessel') && normalized.includes('terminal') && normalized.includes('etb') && normalized.includes('etd')) return i; } return -1; }
function indexOfHeader(headers, candidates) { const normalized = headers.map(normalize); for (const candidate of candidates) { const index = normalized.indexOf(normalize(candidate)); if (index >= 0) return index; } return -1; }
...
function recordFromRow(row, index) {
  ...
  return {
    terminal: get('terminal'), vessel: get('vessel'), voyageImp: get('voyageImp'), voyageExp: get('voyageExp'), service: get('service'), prev: get('prev'), next: get('next'),
...
function parseRows(rows) {
  ...
  const index = {
    terminal: indexOfHeader(headers, ['terminal']), vessel: indexOfHeader(headers, ['vessel']), voyageImp: indexOfHeader(headers, ['imp']), voyageExp: indexOfHeader(headers, ['exp']), service: indexOfHeader(headers, ['service']), prev: indexOfHeader(headers, ['prev']), next: indexOfHeader(headers, ['next']), etb: indexOfHeader(headers, ['etb']), etd: indexOfHeader(headers, ['etd'])
  };
  const records = [];
...
function parseText(text) { return parseRows(text.split('\n').map(line => line.replace(/\r/g, '')).filter(line => line.trim()).map(line => line.includes('\t') ? line.split('\t').map(clean) : line.split(/\s{2,}/).map(clean))); }

async function processFile() {
  const file = schematicFile.files?.[0];
  if (!file) { alert('Selecciona primero el archivo Excel del schematic.'); return; }
...
function getRecordsBetween(start, end) { return currentSchematic.map(item => ({ ...item, etbDateObject: item.etbIso ? new Date(item.etbIso) : parseDateTime(item.etb) })).filter(item => item.etbDateObject && item.etbDateObject >= start && item.etbDateObject <= end).sort((a,b) => a.etbDateObject - b.etbDateObject); }
function getRecordsByVessel(query) { const term = normalize(query); if (!term) return []; return currentSchematic.map(item => ({ ...item, etbDateObject: item.etbIso ? new Date(item.etbIso) : parseDateTime(item.etb) })).filter(item => normalize(item.vessel).includes(term)).sort((a,b) => (a.etbDateObject?.getTime?.() || 0) - (b.etbDateObject?.getTime?.() || 0)); }
function hasImpVoyage(item) { return Boolean(clean(item?.voyageImp)); }
function filterMsc(records) { return records.filter(hasImpVoyage); }

function buildReport(records, title = 'Schematic') {
  const lines = [title, ''];
  if (!records.length) { lines.push('Sin buques programados para el periodo.'); return lines.join('\n'); }
...
function generateRangeReport(onlyMsc = false) {
  ...
  if (rawEnd < start) { alert('La fecha fin no puede ser menor a la fecha inicio.'); return; }
...
function generateVesselReport() {
  ...
  vesselOutput.value = buildReport(records, `Schematic - Consulta por buque: ${query.toUpperCase()}`);
}

async function copyTextFrom(textarea, emptyMessage) {
  const text = textarea.value.trim();
  if (!text) { alert(emptyMessage); return; }
  try { await navigator.clipboard.writeText(textarea.value); alert('Reporte copiado al portapapeles.'); }
  catch { textarea.select(); document.execCommand('copy'); alert('Reporte copiado al portapapeles.'); }
}
...
function hydrateFromStorage() { const payload = loadSchematic(); if (payload?.records?.length) { currentSchematic = payload.records; updateStatus(payload); } }
...
btnVesselSearch.addEventListener('click', generateVesselReport);
